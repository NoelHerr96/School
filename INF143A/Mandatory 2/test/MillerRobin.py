...
# command-line arg.  If you call it with no args, it returns primality resultson a
...
def test_integer_for_prime(p):  # (A1)
    ...
    if any([p % a == 0 for a in probes]):
        return 0  # (A5)
    # need to represent p-1 as  q * 2^k                 #(A6)
    k, q = 0, p - 1
    while not q & 1:  # (A7)
        q >>= 1  # (A8)
        k += 1  # (A9)
    ...
    if len(sys.argv) == 1:  # (M3)
        for p in primes:  # (M4)
            ...
            if probability_of_prime > 0:  # (M7)
                print("%d is prime with probability: %f" %
                      (p, probability_of_prime))
            else:  # (M9)
